...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        Button historyButton = new Button("State History");
        VBox.setMargin(historyButton, insets);

        historyButton.setOnAction(event -> {
            Stage historyWindow = new Stage();
            VBox vBox = new VBox(new Label("History of states"));


            if (!controller.getHistory().isEmpty()){
                controller.getHistory().forEach(memento -> {
                    String stateTimeStamp = memento.getTimeStamp().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
                    Label historyTimeLabel = new Label(stateTimeStamp);

                    historyTimeLabel.setOnMouseClicked(event1 -> {
                        controller.restoreState(memento);
                        updateGui();
                    });

                    vBox.getChildren().add(historyTimeLabel);
                });
            } else {
                vBox.getChildren().add(new Label("History is empty"));
            }

            Scene newScene = new Scene(vBox, 200, 100);
            historyWindow.setScene(newScene);
            historyWindow.setTitle("New Window");
            historyWindow.show();
        });
